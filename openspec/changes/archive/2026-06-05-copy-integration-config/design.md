...
- **Risk**: Clipboard API blocked on non-secure origins.
  - _Mitigation_: Ensure fallback UI behavior or standard browser exceptions are handled; however, the dashboard is hosted on HTTPS (covered by Cloudflare Universal SSL) so it is secure.
