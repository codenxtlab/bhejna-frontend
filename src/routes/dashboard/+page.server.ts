    ...
    initializeOnboarding: async ({ locals }) => {
        ...
        try {
            ...
            if (currentTenant) {
                ...
                if (updateError || !updatedTenant) return fail(500, { message: updateError?.message || 'Update failed' });
                ...
                if (insertError || !newTenant) return fail(500, { message: insertError?.message || 'Insert failed' });
            ...
            const goPayload = {
                id: tenantId,
                waba_id: tenantData.waba_id?.trim() || "",
                phone_number_id: tenantData.phone_number_id?.trim() || "",
                api_key: tenantData.api_key?.trim() || "",
                whatsapp_status: 'PENDING_ONBOARDING',
            ...
            return fail(500, { message: err.message || 'An unexpected internal routing exception occurred' });
        }

        // 3. Return the Meta onboarding URL so the client can open it in a new tab
        const metaHostedOnboardingUrl = 'https://business.facebook.com/messaging/whatsapp/onboard/?app_id=1594349876023947&config_id=1298428131650108';
        return { success: true, onboardingUrl: metaHostedOnboardingUrl };
    },
    ...
    retryWabaSubscription: async ({ locals }) => {
        ...
        try {
            ...
            if (!tenant.waba_id || tenant.waba_id.trim() === '') {
                return fail(400, { message: 'Cannot retry sync: Configuration missing valid WABA ID parameters.' });
            }

            const goPayload = {
                id: tenant.id,
                waba_id: tenant.waba_id.trim(),
                phone_number_id: tenant.phone_number_id?.trim() || "",
                api_key: tenant.api_key?.trim() || "",
            ...
            return fail(500, { message: err.message || 'An unexpected internal sync routing exception occurred' });
        }
    }
};
