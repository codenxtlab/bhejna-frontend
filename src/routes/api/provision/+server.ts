...
export const POST = async ({ request, locals }: RequestEvent): Promise<Response> => {
	...
	try {
		...
		const goPayload = {
			id: newTenant.id,
			waba_id: newTenant.waba_id?.trim() || undefined,
			phone_number_id: newTenant.phone_number_id?.trim() || undefined,
			api_key: newTenant.api_key?.trim() || undefined, // CRITICAL: Use api_key, NOT access_token
